    ...
    render() {
        ...
        return (
            <div className="table-action">
                {
                    this.props.status == 0 ? '' :
                        <Popconfirm title={"是否确定" + statueForTitle + "?"} onConfirm={() => { this.props.changeStatus(this.props.dataId, this.props.status, this.props.eventSourceId) }} onCancel={() => { }} okText="确定" cancelText="取消">
                ...
                {/* <span style={{ margin: '-3px 0 0 5px', cursor: 'pointer' }}>灰</span> <span style={{ margin: '-3px 0 0 5px', cursor: 'pointer' }}>挑</span> */}
            </div>
        )
    }
}
