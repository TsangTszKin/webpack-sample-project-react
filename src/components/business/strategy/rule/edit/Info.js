...
import { Collapse, Table, Button, message, Input, Select, Spin, Icon } from 'antd';
...
class Info extends Component {
    ...
    save = () => {
        common.loading.show();
        strategyService.saveRule(this.saveData).then(res => {
            common.loading.hide();
            if (!publicUtils.isOk(res)) return
            ...
            if (!this.props.match.params.id) {
                message.success('保存成功', 1, () => this.props.history.replace({ pathname: '/business/strategy/rule/save/' + res.data.result.id }));
            } else {
    ...
    verify() {
        ...
        if (this.saveData.code.indexOf("r_") !== 0) {
            message.warning('基本信息 - 标识 必须以 r_ 开头');
            return
        }
        ...
        if (this.state.model === 0) {
            for (let i = 0; i < this.saveData.tempVars.length; i++) {
                ...
                if (element.defaultValue.length > 30) {
                    message.warning('临时变量 - 变量默认值 不能超过个字符');
                    return
                }
        ...
        if (isBatch) {
            ...
            switch (key) {
                ...
                case 'code':
                    ...
                    break;
                case 'defaultValue':
                    this.saveData.defaultValue = this.saveData.defaultValue.substr(0, 255);
                    break;
                case 'description':
    ...
    getRuleById(id) {
        strategyService.getRuleById(id).then(res => {
            ...
            sessionStorage.ruleType = data.type;
            ...
            if (!common.isEmpty(data.treeNode)) {
            ...
            this.saveData.tempVars = !common.isEmpty(data.tempVars) ? data.tempVars : [];
            if (!common.isEmpty(data.filterRuleId)) {
                    ...
                    <Spin spinning={this.state.isLoading} size="large">
                        <FormBlock header="基本信息">
                            <Form>
                                ...
                                <FormItem name="标识" placeHolder="请输入以r_开头的标识" disabled={this.props.match.params.id ? true : false} type="input" isNotNull={true} changeCallBack={this.updateSaveData} code="code" defaultValue={this.saveData.code}></FormItem>
                                ...
                                <FormItem name="规则类型" disabled={this.props.match.params.id ? true : false} type="select" isNotNull={true} changeCallBack={this.updateSaveData} code="type" selectData={[{ code: 0, value: "普通规则" }, { code: 1, value: '过滤规则' }]} defaultValue={this.saveData.type}></FormItem>
                                <FormItem name="类别" type="select" isNotNull={true} changeCallBack={this.updateSaveData} code={['category', 'categoryName']} categoryType="rule" defaultValue={this.saveData.category + '·-·' + this.saveData.categoryName}></FormItem>
