 ...
 * @LastEditTime: 2018-11-16 15:01:39
 * @Description: 
 */
    ...
    @action.bound getDataSourceForApiCallback(res) {
        ...
        if (!common.isEmpty(res.data.result.resultList)) {
            for (let i = 0; i < res.data.result.resultList.length; i++) {
                ...
                data.action = <TableAction dataId={element.id} status={element.status} deleteOne={this.deleteOne} editPath={{ pathname: '/business/strategy/rule-set/save/' + element.type + '/' + element.id }} />;
                dataList.push(data);
            }
        } else {
